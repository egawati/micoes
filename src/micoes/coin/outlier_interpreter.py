...
Copied from https://github.com/ninghaohello/Contextual-Outlier-Interpreter by Ninghao Liu
"""
...
class OutlierInterpreter(object):
    ...
    def interpret_outliers(self, ids_target, sgnf_vec, int_flag=0):
        ...
        score_attr_mat = []
        for i in ids_target:
            # print("Examining outlier points at index {}".format(i))
            # Do clustering on the context, build one classifier for each cluster

            nums_c, clfs, cluster_attr_scale, best_k_info = self.cluster_context(i, int_flag)
        ...
        #     o_dictionary[str(i)] = {}
        #     o_dictionary[str(i)]["best_k_info"] = best_k_info
        #     o_dictionary[str(i)]["score_attr"] = score_attr.tolist()
        #     o_dictionary[str(i)]["outlier_score"] = devt_i
        #     o_dictionary[str(i)]["thepoint"] = self.data[i].tolist()

        # o_dictionary["number_of_outliers"] = len(ids_target)

        return np.array(score_attr_mat), oid_devt_dict, o_dictionary

    def cluster_context(self, id_outlier, int_flag):

        best_k_info = {}  # ega add this
        ...
        # print("Outlier neighbors is {}".format(otlr_nbrs))

        # ega added the first condition
        if otlr_nbrs.shape[0] <= self.MIN_CLUSTER_SIZE:
            k_best = self.DEFK
        elif self.DEFK == 0:
        ...
        clfs = []
        nbrs_mean = []
        nums_c = []  # the number of points in insts_c0 or insts_c1
        cluster_attr_scale = []
        ...
        for c in range(k_best):
            ...
            # ega's code
        #     best_k_info[str(c)] = {}
        #     best_k_info[str(c)]["insts_c0"] = insts_c0.tolist()
        #     best_k_info[str(c)]["insts_c1"] = insts_c1.tolist()
        #     best_k_info[str(c)]["clf.coef_"] = clf.coef_.tolist()

        # best_k_info["ncluster"] = k_best
        # best_k_info["nusedcluster"] = used_cluster
        # best_k_info["nums_c"] = nums_c

        return nums_c, clfs, cluster_attr_scale, best_k_info
