...
from micoes.explainer import ClustreamExplainer
from micoes.microclusters.clustream import CluStream
from micoes.multiprocessing import run_detector_clustream
...
def run_detector_explainer_clustream_online_wtiming(clu, X, detector_type='lof', features=None,
  ...
  # run the outlier detection and clu_stream in parallel
  # logging.info('Run outlier detection and online microcluster')
  detector_clustream_rs, multiprocessing_duration = run_detector_clustream(X, clu, detector_type, profiling=True)
  # get the explaination_results
  results = get_detector_explainer_clustream_result(clu, detector_clustream_rs, X,
                                                    features=features,
  ...
  if isinstance(detector_clustream_rs[1], float):  # thread 1 was for outlier detection
    detection_results = detector_clustream_rs[0][0]
  else:  # thread 1 was for microcluster generation
    detection_results = detector_clustream_rs[1][0]
...
def get_detector_explainer_clustream_result(clu, detector_clustream_rs, X, features=None,
                                            max_rad_multiplier=2, round_flag=True, prior_knowledge=1,
                                            regularization='l1', regularization_param=1,
                                            intercept_scaling=1, simple_feature_contribution=True):
  results = {}
  if isinstance(detector_clustream_rs[1], float):  # thread 1 was for outlier detection
    detection_results = detector_clustream_rs[0][0]
    results['detection_duration'] = detector_clustream_rs[0][1]
    results['microcluster_duration'] = detector_clustream_rs[1]
  else:  # thread 1 was for microcluster generation
    detection_results = detector_clustream_rs[1][0]
    results['detection_duration'] = detector_clustream_rs[1][1]
    results['microcluster_duration'] = detector_clustream_rs[0]

  # run the clustream microcluster based explanation
  # logging.info('Run clu microcluster based xplanation')
  start = time.perf_counter()
  outlier_indices = np.where(detection_results == 1)[0]
  # rads = [mc.get_radius() for mc in clu.microclusters]
  # max_distance = max(rads) * max_rad_multiplier

  max_distance = np.std(X) * max_rad_multiplier

  explainer = ClustreamExplainer(clu.microclusters, max_distance, clu.n_attributes,
                                 features=features,
                                 regularization=regularization,
                                 regularization_param=regularization_param,
                                 intercept_scaling=intercept_scaling,
...
def run_cluexplainer_experiment_on_each_file(filename, n_microclusters, alpha, tau,
  ...
  # logging.info(f'>>n_attributes {n_attributes}')
  ...
  X = data[init:, :]
  result = run_detector_explainer_clustream_online_wtiming(clu, X,
                                                           features=features,
...
def build_dataframe_cluexplainer_experiments_basic(folder, filenames, n_microclusters, alpha, tau,
  ...
  for filename in filenames:
    ...
    result = run_cluexplainer_experiment_on_each_file(filepath, n_microclusters, alpha, tau,
                                                      label=label,
  ...
  experiments['clu_explainer_duration'] = np.sum((np.maximum(detection_duration, microcluster_duration), explanation_duration), axis=0).tolist()
...
def build_compact_df_clu_explainer_experiments(df):
  new_df = df.copy(deep=True)
  new_df['ratio_microcluster_detection'] = df['microcluster_duration'] / df['detection_duration']
  new_df['ratio_clu_explanation_detection'] = df['clu_explainer_duration'] / df['detection_duration']
  new_df['ratio_coin_detection'] = df['coin_duration'] / df['detection_duration']
  return new_df


def simulate_clustream_explainer(clu, detector_clustream_rs, X, window_size, nth_window,
                                 features=None,
                                 max_rad_multiplier=2, round_flag=True, prior_knowledge=1,
                                 regularization='l1', regularization_param=1,
                                 intercept_scaling=1, simple_feature_contribution=True):
